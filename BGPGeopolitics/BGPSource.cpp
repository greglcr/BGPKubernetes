...
BGPSource::BGPSource(BGPMessagePool *bgpMessagePool, unsigned int t_start, unsigned int t_end, unsigned int dumpDuration, map<std::string, unsigned short int> &collectors, std::string &captype, int version, std::string pathCache) :bgpMessagePool(bgpMessagePool), t_start(t_start), t_end(t_end), dumpDuration(dumpDuration), version(version), collectors(collectors), pathCache(pathCache){
    if (captype=="R")
        mode =0;
    else
        mode=1;
...
int BGPSource::run() {
	...
	cout<<"Begin to get a full dump"<<endl;
        ...
        /* Configure the broker interface options */
          ...
          bgpstream_set_data_interface_option(bs, option, (this->pathCache).c_str());
