...
public:
    Wrapper(unsigned int t_start, unsigned int t_end, unsigned int dumpDuration, std::map<std::string, unsigned short int>& collectors ,  std::string& captype, int version, std::string& path, std::string& ppath,int port, int dbase, std::string pathCache, std::string ipServers) {
        Redis *_redis;
        ...
        ShardedBGPRedis *bgpRedis= new ShardedBGPRedis(ipServers, port, dbase,numShards,poolSize);
        ...
        BGPSource *bgpsource = new BGPSource(&bgpMessagePool, t_begin, t_end, dumpDuration, collectors ,  captype, 4, pathCache);
...
int main(int argc, char **argv) {
    ...
    std::string ipServers;
    if( argc > 2 ) {
        string command1(argv[1]);  
        if (command1 == "-h" || command1 == "--help") {
        	std::cout << "Usage : " << std::endl;
        	std::cout << "./BGBGeopolitics -f <configFile> : Give a config file with the json format, see config file example for more details" << std::endl;
        	std::cout << "./BGBGeopolitics -T start end -P path -PP ppath -PORT port -DB dbnumber" << std::endl;
        	exit(0);
        }   
        else if (command1 == "-f") {
    ...
    Wrapper *w = new Wrapper(start, end, dumpDuration, collectors, mode,4, path, ppath, port, dbase, pathCache, ipServers);
    return 0;
}
